...
Do not copy values from a local showcase into this record without an enterprise
decision. Do not store passwords, tokens, private keys, production payloads, or other
Secret values here.
...
A local adapter, test harness, or fixture protocol is not a public enterprise contract.
...
must be pullable from the approved path. A connected workstation does not prove that a
...
actual image store and namespace. A similarly tagged image in a workstation cache is
...
Provision Secret contracts before dependent workloads. Never alternate generic fixture
...
## Local Evidence Translation

Local development proves repository behavior under controlled substitutes. Translate
each local input before using it in a fielded installation.

| Local or reference input | Enterprise decision required |
|---|---|
| k3d cluster | Supported enterprise Kubernetes platform and ownership |
| Loopback origin and port-forward | Canonical DNS, ingress, TLS, proxy, and callback path |
| Local Keycloak fixture | Enterprise identity provider, clients, claims, and lifecycle |
| Generated local credentials | Enterprise Secret authority, registration, rotation, and projection |
| Local OCI registry | Approved registry, mirrors, retention, scanning, and transfer |
| Direct Helm command | GitOps or approved direct-release ownership |
| Workstation image cache | Node-visible registry and runtime image-store closure |
| Fixture namespace and labels | Installation-owned namespace, naming, and selectors |
| Showcase role names | Enterprise job roles and separation of duties |
| Example Work Context | Approved business context and membership |
| Local PVC | Enterprise storage class, backup, restore, and lifecycle |
| Development cleanup | Approved environment-specific retention and decommission procedure |
| Test harness credential | Supported identity input owned by the acceptance procedure |
| Example defect workaround | Explicit product limitation or independently approved product fix |

No local hostname, credential, identity, image cache, port, storage path, cleanup command,
or manual repair becomes an enterprise default.
...
## Reference-Learning Coverage

This table records why the readiness controls exist. It transfers lessons from a local
enterprise simulation without turning its topology or fixture values into customer
requirements.
...
| Example configuration and product behavior have different owners | Use the failure classification and defect register |
| Local identity is a test substitute | Select the enterprise issuer, clients, claims, proxy, and lifecycle explicitly |
...
| Real responses can contain more fields than a narrow fixture model | Validate complete typed responses and unknown-field policy |
...
| A workstation may resolve a registry while a cluster node cannot | Test node and Pod DNS and registry paths independently |
